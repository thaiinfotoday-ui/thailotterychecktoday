...
export async function getHistoryData(year = null, page = 1) {
    // 1. Start with static data
    let data = [...staticHistoryData];

    // 2. Try to fetch the absolutely latest LIVE data to see if we have a new draw
    try {
        const liveData = await getLotteryData();
